...
1. From the project root, run `do src/01_clean_prep_PPP.do` in Stata to set paths, clean the parsed PPP borrower file, and set up data for all imputation methods
   - **Note:** Before running this, make sure the support files listed in the data section above are saved in the `Data/` directory.
2. Run `src/02_predict_PPP.Rmd` to run all imputation methods that use R *(Last Name, WRU/BISG, BIFSG, BIRDiE)*
3. To run *ZRP*,
   - run `python src/sub/PPP_ZRP_predict.py`
   - this uses `Data/PPP_ZRP_interim.csv`
   - it writes `Data/PPP_results_interim_ZRP.csv`
4. To run *NamePrism*,
...
- run for `04_NamePrism_predict_PPP.py`: about 0.9 second per name, 70 hours in totle
